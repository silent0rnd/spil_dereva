...
export default function CalculatorComponent({ onOpenCallbackModal }: CalculatorProps) {
  ...
  // Soft flash on the result card so a recalculation is noticed.
  useEffect(() => {
    if (shouldReduceMotion || estimatedPrice === 0) return;
    flashControls.start({
      opacity: [0, 0.55, 0],
      transition: { duration: 0.6, ease: 'easeOut' }
    });
  }, [estimatedPrice, flashControls, shouldReduceMotion]);

  const activeService = SERVICES.find(s => s.id === params.serviceId) || SERVICES[0];
  const isTreeService = ['felling-whole', 'felling-parts', 'pruning'].includes(params.serviceId);
  const isStumpOnly = params.serviceId === 'stump-removal';

  const diameterPercent = ((params.diameter - MIN_DIAMETER) / (MAX_DIAMETER - MIN_DIAMETER)) * 100;
  ...
  return (
    <section id="calculator" className="py-20 md:py-28 bg-white border-y border-ink-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        <SectionHeading
          id="calculator-header"
          eyebrow="Онлайн-оценка"
          title="Калькулятор стоимости работ"
          subtitle="Укажите параметры дерева, чтобы получить мгновенную прозрачную оценку. Цена является ориентировочной и фиксируется при заключении официального договора."
          className="mb-16"
        />
        ...
        <div id="calculator-grid" className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
          ...
          <Reveal className="lg:col-span-7">
            <div
              ...
              className="bg-forest-50 border border-forest-100 rounded-panel p-6 md:p-8 space-y-8"
            >

              {/* Step 1: Service Selection */}
              <div>
                <label id="label-service" className="block text-sm font-bold text-ink-900 mb-3.5 uppercase tracking-wider">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {SERVICES.map((s) => (
                    <button
                      ...
                      className={`p-3.5 text-left rounded-btn border text-xs font-bold transition-all flex items-center space-x-2.5 cursor-pointer ${
                        params.serviceId === s.id
                          ? 'bg-forest-600 border-forest-600 text-white shadow-card'
                          : 'bg-white border-ink-200 text-ink-700 hover:border-forest-300'
                      }`}
                    >
              ...
              {(isTreeService || isStumpOnly) && (
                <div>
                  <div className="flex justify-between items-center mb-3">
                    <label id="label-diameter" className="block text-sm font-bold text-ink-900 uppercase tracking-wider">
                      2. Примерный диаметр ствола
                    </label>
                    <span className="text-base font-bold font-mono text-forest-700 bg-forest-100 px-3 py-1 rounded-btn">
                      {params.diameter} см
                    </span>
                  </div>
                  ...
                  <input
                    ...
                    aria-label="Примерный диаметр ствола в сантиметрах"
                    className="w-full h-2 rounded-full appearance-none cursor-pointer accent-forest-600"
                    style={{
                      background: `linear-gradient(to right, var(--color-forest-600) ${diameterPercent}%, var(--color-ink-200) ${diameterPercent}%)`
                    }}
                  />

                  <div className="flex justify-between text-[11px] text-ink-500 mt-2 font-mono">
              ...
              {isTreeService && (
                <div>
                  <label id="label-proximity" className="block text-sm font-bold text-ink-900 mb-3.5 uppercase tracking-wider">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    {[
                      ...
                      <button
                        ...
                        className={`p-3 text-left rounded-btn border transition-all cursor-pointer ${
                          params.distanceToBuildings === opt.id
                            ? 'bg-forest-100 border-forest-600 text-forest-900 shadow-card'
                            : 'bg-white border-ink-200 text-ink-700 hover:border-forest-300'
                        }`}
                      >
                        <span className="block text-xs font-bold">{opt.label}</span>
                        <span className="block text-[10px] text-ink-500 font-light mt-0.5">{opt.desc}</span>
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div>
                <label id="label-addons" className="block text-sm font-bold text-ink-900 mb-3.5 uppercase tracking-wider">
                  {isTreeService ? '4. Сопутствующие услуги' : '2. Дополнительные опции'}
                </label>
                <div className="space-y-3">
                  {/* Power Lines Difficulty Toggle (only for trees) */}
                  {isTreeService && (
                    <label id="toggle-power-lines" className="flex items-start p-3 bg-white rounded-btn border border-ink-100 shadow-card cursor-pointer select-none hover:border-forest-200 transition-colors">
                      <input
                        type="checkbox"
                        checked={params.hasPowerLines}
                        onChange={(e) => setParams(p => ({ ...p, hasPowerLines: e.target.checked }))}
                        className="w-4 h-4 text-forest-600 border-ink-300 rounded focus:ring-forest-500 mt-0.5"
                      />
                      <div className="ml-3 text-xs">
                        <span className="block font-bold text-ink-900">Близость проводов ЛЭП</span>
                        <span className="block text-[10px] text-ink-500 font-light mt-0.5">Требует привлечения арбористов со спецдопуском или оттяжки ветвей (+1 500 ₽)</span>
                      </div>
                    </label>
                  )}
                  ...
                  {isTreeService && (
                    <label id="toggle-stump-removal" className="flex items-start p-3 bg-white rounded-btn border border-ink-100 shadow-card cursor-pointer select-none hover:border-forest-200 transition-colors">
                      <input
                        type="checkbox"
                      ...
                      />
                      <div className="ml-3 text-xs">
                        <span className="block font-bold text-ink-900">Выкорчевать / сдробить пень</span>
                        <span className="block text-[10px] text-ink-500 font-light mt-0.5">Дробление пня фрезой ниже уровня земли на 30 см (от +1 000 ₽)</span>
                      </div>
                    </label>
                  )}

                  {/* Cleanup Toggle */}
                  <label id="toggle-cleanup" className="flex items-start p-3 bg-white rounded-btn border border-ink-100 shadow-card cursor-pointer select-none hover:border-forest-200 transition-colors">
                    <input
                      type="checkbox"
                    ...
                    />
                    <div className="ml-3 text-xs">
                      <span className="block font-bold text-ink-900">Сбор мусора и вывоз порубочных остатков</span>
                      <span className="block text-[10px] text-ink-500 font-light mt-0.5">Соберем все ветки, распилим ствол на дрова, уберем опилки и вывезем самосвалом (от +1 500 ₽)</span>
                    </div>
                  </label>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-forest-200">
                <span className="text-xs font-bold text-ink-900 uppercase tracking-wider">Количество объектов</span>
                <div className="flex items-center space-x-3 bg-white border border-ink-200 rounded-btn p-1">
                  <button
                    id="qty-decrement"
                    type="button"
                    onClick={() => setParams(p => ({ ...p, count: Math.max(1, p.count - 1) }))}
                    aria-label="Уменьшить количество"
                    className="w-8 h-8 flex items-center justify-center font-bold text-ink-600 hover:bg-ink-50 rounded-btn transition-colors cursor-pointer"
                  >
                    -
                  </button>
                  <span className="text-sm font-bold font-mono text-ink-800 w-6 text-center">{params.count}</span>
                  <button
                    id="qty-increment"
                    type="button"
          ...
          <Reveal delay={0.1} className="lg:col-span-5 lg:sticky lg:top-24">
            ...
            <div id="calculator-results" className="relative z-10">
              ...
              <div className="relative bg-forest-900/50 rounded-card p-5 mb-8 border border-forest-800/30">
                {/* Recalculation flash */}
                <motion.span
                  aria-hidden="true"
                  initial={{ opacity: 0 }}
                  animate={flashControls}
                  className="pointer-events-none absolute inset-0 rounded-card ring-2 ring-forest-400"
                />
                ...
                <div className="flex items-baseline space-x-1.5 mt-2">
                  <span className="text-3xl md:text-4xl font-bold font-mono text-white tabular-nums">
                    ~ <CountUp value={estimatedPrice} startOnView={false} />
                  </span>
              ...
              <button
                ...
                className="w-full py-4 bg-forest-500 hover:bg-forest-400 text-white font-bold text-xs uppercase tracking-wider rounded-btn shadow-panel transition-all flex items-center justify-center space-x-2 hover:-translate-y-0.5 active:translate-y-0 active:scale-95 cursor-pointer"
              >
